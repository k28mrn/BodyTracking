...
public struct Body : ISerializable
{
    ...
    //カラーカメラの2D情報
    public System.Numerics.Vector2[] JointPositions2DColor;
    ...
    public Body(int maxJointsLength)
    {
        ...
        JointPositions2DColor = new System.Numerics.Vector2[maxJointsLength];
    ...
    public static Body DeepCopy(Body copyFromBody)
    {
        ...
        for (int i = 0; i < maxJointsLength; i++)
        {
            ...
            copiedBody.JointPositions2DColor[i] = copyFromBody.JointPositions2DColor[i];
    ...
    public void CopyFromBodyTrackingSdk(Microsoft.Azure.Kinect.BodyTracking.Body body, Calibration sensorCalibration)
    {
        ...
        for (int bodyPoint = 0; bodyPoint < Length; bodyPoint++)
        {
            ...
            if (position2d != null)
            {
                ...
                //カラーカメラでの2D情報
                var position2dColor = sensorCalibration.TransformTo2D(
                    position2d.Value,
                    1000.0f,
                    CalibrationDeviceType.Depth,
                ...
                if (position2dColor != null)
                {
                    JointPositions2DColor[bodyPoint] = position2dColor.Value;
                }
                else
                {
                    JointPositions2DColor[bodyPoint].X = Invalid2DCoordinate;
                    JointPositions2DColor[bodyPoint].Y = Invalid2DCoordinate;
                }
    ...
    public Body(SerializationInfo info, StreamingContext context)
    {
        ...
        float[] JointPositions2DColor_X = (float[])info.GetValue("JointPositions2DColor_X", typeof(float[]));
        float[] JointPositions2DColor_Y = (float[])info.GetValue("JointPositions2DColor_Y", typeof(float[]));
        JointPositions2DColor = new System.Numerics.Vector2[JointPositions2DColor_X.Length];
        for (int i = 0; i < JointPositions2D_X.Length; i++)
        {
            JointPositions2DColor[i].X = JointPositions2DColor_X[i];
            JointPositions2DColor[i].Y = JointPositions2DColor_Y[i];
        }
